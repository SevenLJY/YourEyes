 ...
 */
public class PasswordLoginActivity extends Activity implements GestureDetector.OnGestureListener{
    ...
    VoiceUtil voiceUtil;
    String message = "登录。请输入密码，右滑语音输入，左滑删除，上滑确认，下滑返回.强烈建议您在家人朋友的帮助下进行登录。";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        SpeechUtility.createUtility(PasswordLoginActivity.this, SpeechConstant.APPID+"=58593564");
        voiceUtil = new VoiceUtil(PasswordLoginActivity.this);
    ...
    private void test(int result){
        switch(result){
            case 0:
                FixedValue.pswChanged = false;
                editor.putString("username",username);
                editor.putString("password",password);
                editor.commit();
                voiceUtil.stopSpeak();
                voiceUtil.speak("开始登录");
                FixedValue.username = username;
                Intent intent = new Intent(PasswordLoginActivity.this, MapActivity.class);
            ...
            case 3:
                voiceUtil.stopSpeak();
                voiceUtil.speak("用户名与密码不匹配，请返回重新登录。");
                break;
            case 4:
                voiceUtil.stopSpeak();
