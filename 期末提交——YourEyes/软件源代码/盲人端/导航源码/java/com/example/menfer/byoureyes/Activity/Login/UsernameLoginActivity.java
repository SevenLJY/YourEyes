 ...
 */
public class UsernameLoginActivity extends Activity implements GestureDetector.OnGestureListener{
    EditText et_username;
    GestureDetector detector;
    VoiceUtil voiceUtil;

    String message = "登录。请输入用户名，右滑语音输入，左滑删除，上滑确认，下滑返回.强烈建议您在家人朋友的帮助下进行登录。";


    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        SpeechUtility.createUtility(UsernameLoginActivity.this, SpeechConstant.APPID+"=58593564");
        setContentView(R.layout.activity_username_register);
        voiceUtil = new VoiceUtil(UsernameLoginActivity.this);
        if (FixedValue.pswChanged){
            message = "密码已被修改，跳转至登录，输入用户名界面。"+message;
        }
    ...
    public boolean onFling(MotionEvent motionEvent, MotionEvent motionEvent1, float v, float v1) {
        if(motionEvent.getX()-motionEvent1.getX()>100){
            ...
            voiceUtil.stopSpeak();
            voiceUtil.speak("输入的用户名为"+ "[n1]"+username + ",确认注册请长[=chang2]按屏幕");
        }else if(motionEvent.getY()-motionEvent1.getY()<-100) {
            //向下滑的操作，返回
            try
            {
    ...
    public void onLongPress(MotionEvent motionEvent) {
        ...
        if(!Check.uesrname_stringFilter(username)){
            ...
            Intent intent = new Intent(UsernameLoginActivity.this, PasswordLoginActivity.class);
