 ...
 */
public class AgeModifyInfoActivity extends Activity implements GestureDetector.OnGestureListener{
    EditText et_age;
    GestureDetector detector;
    VoiceUtil voiceUtil;

    String message = "修改年龄，请输入十到一百之间的数字，右滑语音输入，左滑删除，上滑确认，下滑返回.强烈建议您在家人朋友的帮助下进行修改信息。";


    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        SpeechUtility.createUtility(AgeModifyInfoActivity.this, SpeechConstant.APPID+"=58593564");
        setContentView(R.layout.activity_age);
        voiceUtil = new VoiceUtil(AgeModifyInfoActivity.this);
    ...
    public boolean onFling(MotionEvent motionEvent, MotionEvent motionEvent1, float v, float v1) {
        if(motionEvent.getX()-motionEvent1.getX()>100){
            ...
            //向右滑的操作 语音输入
            voiceUtil.voiceInput(et_age);
        }else if(motionEvent.getY()-motionEvent1.getY()>100){
            //向上滑的操作 确认
            String age = et_age.getText().toString();
            voiceUtil.stopSpeak();
            voiceUtil.speak("输入的年龄为"+age + ",确认请长[=chang2]按屏幕");
        }else if(motionEvent.getY()-motionEvent1.getY()<-100) {
            //向下滑的操作，返回
            try
            {
    ...
    public void onLongPress(MotionEvent motionEvent) {
        ...
        try{
            ...
            if (theAge>10&&theAge<100){
                ...
                Intent intent = new Intent(AgeModifyInfoActivity.this, GenderModifyInfoActivity.class);
                intent.putExtras(data);
                startActivity(intent);
            }else {
                voiceUtil.stopSpeak();
                voiceUtil.speak("输入的年龄不合规范，请输入十到一百之间整数。");
            }
        }catch (Exception e){
            voiceUtil.stopSpeak();
            voiceUtil.speak("输入的年龄含不是整数的字符，请输入十到一百之间整数。");
        }
    }
